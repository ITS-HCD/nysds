...
The global stylesheet (`nysds.min.css`) is the foundation of the Excelsior Design System. It defines core design tokens for colors, typography, and spacing — like `--nys-color-theme`, `--nys-space-200`, and `--nys-font-size-md`. NYSDS's web components need these tokens to display correctly.
