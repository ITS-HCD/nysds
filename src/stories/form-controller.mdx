...
import * as FormStories from '../scripts/external/form-controller/src/form-controller.stories';
...
<p className="pill pill--uikit" target="_blank"><a href="https://www.figma.com/design/0ogYpymUPQQfhELthntNbt/%F0%9F%92%A0-NYS-%2F-Excelsior-Components-(Alpha)?node-id=3307-8378&t=FsdGi49PZJ04VzFO-0">Get the UI kit</a></p>

<Canvas of={FormStories.Basic} />

In the example above, we successfully processed the `FormData` object from our `nys-component` by listening to the native form submission event. This allows your form to seamlessly handle both standard HTML form elements and custom `nys-components`.

<div style={{ display: 'flex', gap: '10px', verticalAlign: 'middle', alignItems: 'center' }}>  <nys-icon size="xl" name="info"></nys-icon> It is recommended that users perform a validation check within the submit event before processing form data. This ensures that all required fields and custom form controls are properly validated before submission. </div>
```js
...
### Associating Components with Forms Using the `form` Property
...
This property should only be used when the component is located outside the `<form>` element it needs to interact with.
```html
<!-- For example -->
<nys-form id="my-form">
