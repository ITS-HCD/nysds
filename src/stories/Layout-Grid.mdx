import { Meta } from "@storybook/addon-docs/blocks";


<Meta title="Layout Grid" />

# Layout Grid

Use our flexible grid system to structure website content. The grid is mobile-first, powered by flexbox, and based on a 12-column system.

## How it works

The grid system uses a series of containers, rows, and columns to lay out and align content. The following row and corresponding code are an example of and  in-depth look at how the grid comes together.

<div className="docs-grid-example">

  <div className="nys-grid-container">
    <div className="nys-grid-row">
      <div className="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
      <div className="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
      <div className="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
    </div>
  </div>

</div>

```html
<div class="nys-grid-container">
  <div class="nys-grid-row">
    <div class="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
    <div class="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
    <div class="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
  </div>
</div>
```

<p>This example code creates three equal-width columns on tablet, desktop, and widescreen devices by using our predefined grid classes. Those columns are centered in the page with the parent <code>nys-grid-container</code> container.</p>
<p>The following sections break the layout grid down and describe how it works.</p>
<h3 id="containers-rows-and-columns"><a id="containers-rows-and-columns-2" ></a>Containers, rows, and columns</h3>
  <p><strong>Containers:</strong> <code>nys-grid-container</code> centers the container and gives it a maximum width of <code>nys-desktop</code> (1024px). If you would like the grid to span the full width of the page, do not use <code>nys-grid-container</code>.</p>
    <ul>
      <li><code>nys-grid-container</code> can also accept any breakpoint width: <code>nys-mobile-lg</code>, <code>nys-tablet</code>, and <code>nys-desktop</code>.</li>
      <li>By default, <code>nys-grid-container</code> has a <code>padding-x</code> of 2 at narrow widths, and a <code>padding-x</code> of 4 at <code>nys-desktop</code> and wider.</li>
    </ul>
  <p><strong>Rows:</strong> Columns must have a <code>nys-grid-row</code> as a parent.</p>
  <p><strong>Columns:</strong> <code>nys-grid-col-[1-12]</code> indicates the number of columns the item spans out of a possible 12 per row. So, if you want three equal-width columns across, use <code>nys-grid-col-4</code> for each item.</p>

<h3 id="additional-functionality"><a id="additional-functionality-2" ></a>Additional functionality</h3>
<ul>
  <li><strong>Equal-width columns:</strong> With flexbox, grid columns without a specified width will display as equal-width columns. For example, four instances of <code >grid-col</code> will display as one-quarter-width columns across all sizes. Refer to the <a href="#auto-layout-columns">auto-layout columns</a> section for more examples.</li>
  <li><strong>Gutters:</strong> Rows and columns don't have any gutters by default, but gutters can be added by including a <code>nys-grid-gap</code> modifier (e.g., <code>nys-grid-gap-100</code>, <code>nys-grid-gap-200</code>) at the row level. Refer to <a href="#gutters">gutters</a> for more info.</li>
  <li><strong>Media queries:</strong> Grid breakpoints are based on minimum-width media queries, meaning they apply to that specific width and all greater widths (e.g., <code >tablet:col-4</code> applies to tablet, desktop, and widescreen devices but not at <code >mobile-lg</code> or any width below the tablet breakpoint). Refer to <a href="#responsive-variants">responsive variants</a> for a full list.</li>
</ul>
<h2 id="auto-layout-columns"><a id="auto-layout-columns-2" ></a>Auto layout columns</h2>
<h3><a id="variable-width-content" ></a>Variable-width content</h3>
<p><code>.nys-grid-col-auto</code> items fit the natural width of their content.</p>
<p><code>.nys-grid-col</code> and <code>.nys-grid-col-fill</code> items flex to fill the available space as illustrated in the following example row and code.</p>
<div className="docs-grid-example">


<div className="nys-grid-container">
  <div className="nys-grid-row">
    <div className="nys-grid-col-auto">.nys-grid-col-auto</div>
    <div className="nys-grid-col">.nys-grid-col</div>
    <div className="nys-grid-col-fill">.nys-grid-col-fill</div>
    <div className="nys-grid-col-auto">.nys-grid-col-auto</div>
  </div>
</div>

</div>

```html
<div class="nys-grid-container">
  <div class="nys-grid-row">
    <div class="nys-grid-col-auto">.nys-grid-col-auto</div>
    <div class="nys-grid-col">.nys-grid-col</div>
    <div class="nys-grid-col-fill">.nys-grid-col-fill</div>
    <div class="nys-grid-col-auto">.nys-grid-col-auto</div>
  </div>
</div>
```

<h2 id="responsive-classes"><a id="responsive-classes-2"></a>Responsive classes</h2>
<h3><a id="same-at-all-breakpoints"></a>Same at all breakpoints</h3>
<p>For columns that should maintain the same proportion at any viewport width, use the <code>.nys-grid-col</code> and <code>.nys-grid-col-*</code> classes. Specify a numbered class when you need a column of a specific width; otherwise, use <code>.nys-grid-col</code>.</p>
<p><code>.nys-grid-col-[1-12]</code> sets a fixed width of [n] grid columns in a 12-column grid.</p>

<div className="nys-grid-row border-left">

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">1</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">2</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">3</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">4</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">5</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">6</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">7</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">8</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">9</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">10</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">11</div>
  </div>

  <div className="nys-grid-col-1 border-right">
    <div className="text-center">12</div>
  </div>

</div>

<div className="docs-grid-example">


<div className="nys-grid-row">
  <div className="nys-grid-col-1">.nys-grid-col-1</div>
  <div className="nys-grid-col-2">.nys-grid-col-2</div>
  <div className="nys-grid-col-3">.nys-grid-col-3</div>
  <div className="nys-grid-col-4">.nys-grid-col-4</div>
  <div className="nys-grid-col-2">.nys-grid-col-2</div>
</div>

<div className="nys-grid-row">
  <div className="nys-grid-col-8">.nys-grid-col-8</div>
  <div className="nys-grid-col-2">.nys-grid-col-2</div>
  <div className="nys-grid-col-2">.nys-grid-col-2</div>
</div>

</div>

```html
<div class="nys-grid-row">
  <div class="nys-grid-col-1">.nys-grid-col-1</div>
  <div class="nys-grid-col-2">.nys-grid-col-2</div>
  <div class="nys-grid-col-3">.nys-grid-col-3</div>
  <div class="nys-grid-col-4">.nys-grid-col-4</div>
  <div class="nys-grid-col-2">.nys-grid-col-2</div>
</div>

<div class="nys-grid-row">
  <div class="nys-grid-col-8">.nys-grid-col-8</div>
  <div class="nys-grid-col-2">.nys-grid-col-2</div>
  <div class="nys-grid-col-2">.nys-grid-col-2</div>
</div>
```

<h3><a id="stacked-columns-at-narrow-widths"></a>Stacked columns at narrow widths</h3>
<p>Columns are full-width until the narrowest breakpoint specified in a <code>.nys-grid-col</code> class. For instance, using a single set of <code>tablet:grid-col-*</code> classes, you can create a basic grid system that starts out stacked before displaying as columns at the tablet breakpoint (<code>tablet:</code>) as illustrated in the following rows and corresponding code.</p>

<div className="docs-grid-example">

<div className="nys-grid-row">
  <div className="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
  <div className="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
  <div className="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
</div>

<div className="nys-grid-row">
  <div className="nys-tablet:nys-grid-col-4">.nys-tablet:nys-grid-col-4</div>
  <div className="nys-tablet:nys-grid-col-8">.nys-tablet:nys-grid-col-8</div>
</div>

</div>

```html
<div class="nys-grid-row">
  <div class="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
  <div class="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
  <div class="nys-tablet:nys-grid-col">.nys-tablet:nys-grid-col</div>
</div>

<div class="nys-grid-row">
  <div class="nys-tablet:nys-grid-col-4">.nys-tablet:nys-grid-col-4</div>
  <div class="nys-tablet:nys-grid-col-8">.nys-tablet:nys-grid-col-8</div>
</div>
```

<h3><a id="mix-and-match"></a>Mix and match</h3>
<p>Don't want your columns to simply stack in some breakpoints? Use a combination of different classes for each breakpoint as needed. See the following example rows and corresponding code for a better idea of how it all works.</p>

<div className="nys-grid-row border-left">
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">1</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">2</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">3</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">4</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">5</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">6</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">7</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">8</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">9</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">10</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">11</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">12</div>
  </div>
</div>

<div className="docs-grid-example">


{/* Stack the columns on mobile by making one full-width and the other half-width */}
<div className="nys-grid-row">
  <div className="nys-tablet:nys-grid-col-8">.nys-tablet:nys-grid-col-8</div>
  <div className="nys-grid-col-6 nys-tablet:nys-grid-col-4">.nys-grid-col-6 .nys-tablet:nys-grid-col-4</div>
</div>

{/* Columns start at 50% wide on mobile and bump up to 33.3% wide on desktop */}
<div className="nys-grid-row">
  <div className="nys-grid-col-6 nys-tablet:nys-grid-col-4">.nys-grid-col-6 .nys-tablet:nys-grid-col-4</div>
  <div className="nys-grid-col-6 nys-tablet:nys-grid-col-4">.nys-grid-col-6 .nys-tablet:nys-grid-col-4</div>
  <div className="nys-grid-col-6 nys-tablet:nys-grid-col-4">.nys-grid-col-6 .nys-tablet:nys-grid-col-4</div>
</div>

{/* Columns are always 50% wide, on mobile and desktop */}
<div className="nys-grid-row">
  <div className="nys-grid-col-6">.nys-grid-col-6</div>
  <div className="nys-grid-col-6">.nys-grid-col-6</div>
</div>

</div>

```html
<!-- Stack the columns on mobile by making one full-width and the other half-width -->
<div class="nys-grid-row">
  <div class="nys-tablet:nys-grid-col-8">.nys-tablet:nys-grid-col-8</div>
  <div class="nys-grid-col-6 nys-tablet:nys-grid-col-4">.nys-grid-col-6 .nys-tablet:nys-grid-col-4</div>
</div>

<!-- Columns start at 50% wide on mobile and bump up to 33.3% wide on desktop -->
<div class="nys-grid-row">
  <div class="nys-grid-col-6 nys-tablet:nys-grid-col-4">.nys-grid-col-6 .nys-tablet:nys-grid-col-4</div>
  <div class="nys-grid-col-6 nys-tablet:nys-grid-col-4">.nys-grid-col-6 .nys-tablet:nys-grid-col-4</div>
  <div class="nys-grid-col-6 nys-tablet:nys-grid-col-4">.nys-grid-col-6 .nys-tablet:nys-grid-col-4</div>
</div>

<!-- Columns are always 50% wide, on mobile and desktop -->
<div class="nys-grid-row">
  <div class="nys-grid-col-6">.nys-grid-col-6</div>
  <div class="nys-grid-col-6">.nys-grid-col-6</div>
</div>
```

<h2 id="offsetting-columns"><a id="offsetting-columns-2"></a>Offsetting columns</h2>
<p><code>.nys-grid-offset-[1-12]</code> offsets an item by the specified number of grid columns as shown in the following example.</p>

<div className="nys-grid-row border-left">
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">1</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">2</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">3</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">4</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">5</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">6</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">7</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">8</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">9</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">10</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">11</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">12</div>
  </div>
</div>
<div className="docs-grid-example">


<div className="nys-grid-row">
  <div className="nys-grid-col-8 nys-grid-offset-4">.nys-grid-col-8.nys-grid-offset-4</div>
</div>

</div>

```html
<div class="nys-grid-row">
  <div class="nys-grid-col-8 nys-grid-offset-4">.nys-grid-col-8.nys-grid-offset-4</div>
</div>
```

<h2 id="column-wrapping"><a id="column-wrapping-2"></a>Column wrapping</h2>
<p>Items wrap when the sum of the grid columns is more than 12 as shown in the following example.</p>

<div className="nys-grid-row border-left">
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">1</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">2</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">3</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">4</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">5</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">6</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">7</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">8</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">9</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">10</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">11</div>
  </div>
  <div className="nys-grid-col-1 border-right">
    <div className="text-center">12</div>
  </div>
</div>

<div className="docs-grid-example">


<div className="nys-grid-row">
  <div className="nys-grid-col-8">.nys-grid-col-8</div>
  <div className="nys-grid-col-3">.nys-grid-col-3</div>
  <div className="nys-grid-col-5">.nys-grid-col-5</div>
</div>

</div>

```html
<div class="nys-grid-row">
  <div class="nys-grid-col-8">.nys-grid-col-8</div>
  <div class="nys-grid-col-3">.nys-grid-col-3</div>
  <div class="nys-grid-col-5">.nys-grid-col-5</div>
</div>
```

<h2 id="gutters"><a id="gutters-2"></a>Gutters</h2>
<p>Gap classes are applied to <code>nys-grid-row</code> and come in three axis variants:</p>
<ul>
  <li><code>nys-grid-gap</code> — applies gaps both horizontally (between columns) and vertically (between wrapped rows)</li>
  <li><code>nys-grid-gap-x</code> — horizontal gaps only (between columns)</li>
  <li><code>nys-grid-gap-y</code> — vertical gaps only (between wrapped rows)</li>
</ul>
<p>Each variant also accepts a size suffix from the NYS spacing ramp (e.g. <code>nys-grid-gap-x-200</code>, <code>nys-grid-gap-y-400</code>). Without a suffix, the default gap is <code>0.5rem</code> (1rem at <code>desktop</code> and wider).</p>

<h3 id="default-gutter"><a id="default-gutter-2"></a>Default gap — both axes</h3>
<p>Add <code>nys-grid-gap</code> to apply a horizontal and vertical gutter in one class. The default gap is <code>0.5rem</code> and increases to <code>1rem</code> at <code>desktop</code> and wider.</p>

<div className="docs-grid-example margin-top-2">

<div className="nys-grid-row nys-grid-gap">
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
</div>

</div>

```html
<div class="nys-grid-row nys-grid-gap">
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
</div>
```

<h3 id="horizontal-gap"><a id="horizontal-gap-2"></a>Horizontal gap only</h3>
<p>Use <code>nys-grid-gap-x</code> to add space only between columns, with no vertical gap between wrapped rows.</p>

<div className="docs-grid-example margin-top-2">

<div className="nys-grid-row nys-grid-gap-x">
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
</div>

</div>

```html
<div class="nys-grid-row nys-grid-gap-x">
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
</div>
```

<h3 id="vertical-gap"><a id="vertical-gap-2"></a>Vertical gap only</h3>
<p>Use <code>nys-grid-gap-y</code> to add space only between wrapped rows, with no horizontal gap between columns.</p>

<div className="docs-grid-example margin-top-2">

<div className="nys-grid-row nys-grid-gap-y">
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
</div>

</div>

```html
<div class="nys-grid-row nys-grid-gap-y">
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
</div>
```

<h3><a id="gap-sizes"></a>Gap sizes</h3>
<p>Append a size token to any gap variant. The full ramp, and the class pattern for each axis:</p>

<table>
  <thead>
    <tr>
      <th>Size</th>
      <th>Value</th>
      <th>Both axes</th>
      <th>Horizontal only</th>
      <th>Vertical only</th>
    </tr>
  </thead>
  <tbody>
    <tr><td>1px</td><td>1px</td><td><code>nys-grid-gap-1px</code></td><td><code>nys-grid-gap-x-1px</code></td><td><code>nys-grid-gap-y-1px</code></td></tr>
    <tr><td>2px</td><td>2px</td><td><code>nys-grid-gap-2px</code></td><td><code>nys-grid-gap-x-2px</code></td><td><code>nys-grid-gap-y-2px</code></td></tr>
    <tr><td>50</td><td>4px</td><td><code>nys-grid-gap-50</code></td><td><code>nys-grid-gap-x-50</code></td><td><code>nys-grid-gap-y-50</code></td></tr>
    <tr><td>100</td><td>8px</td><td><code>nys-grid-gap-100</code></td><td><code>nys-grid-gap-x-100</code></td><td><code>nys-grid-gap-y-100</code></td></tr>
    <tr><td>150</td><td>12px</td><td><code>nys-grid-gap-150</code></td><td><code>nys-grid-gap-x-150</code></td><td><code>nys-grid-gap-y-150</code></td></tr>
    <tr><td>200</td><td>16px</td><td><code>nys-grid-gap-200</code></td><td><code>nys-grid-gap-x-200</code></td><td><code>nys-grid-gap-y-200</code></td></tr>
    <tr><td>250</td><td>20px</td><td><code>nys-grid-gap-250</code></td><td><code>nys-grid-gap-x-250</code></td><td><code>nys-grid-gap-y-250</code></td></tr>
    <tr><td>300</td><td>24px</td><td><code>nys-grid-gap-300</code></td><td><code>nys-grid-gap-x-300</code></td><td><code>nys-grid-gap-y-300</code></td></tr>
    <tr><td>400</td><td>32px *</td><td><code>nys-grid-gap-400</code></td><td><code>nys-grid-gap-x-400</code></td><td><code>nys-grid-gap-y-400</code></td></tr>
    <tr><td>500</td><td>40px *</td><td><code>nys-grid-gap-500</code></td><td><code>nys-grid-gap-x-500</code></td><td><code>nys-grid-gap-y-500</code></td></tr>
    <tr><td>600</td><td>48px *</td><td><code>nys-grid-gap-600</code></td><td><code>nys-grid-gap-x-600</code></td><td><code>nys-grid-gap-y-600</code></td></tr>
    <tr><td>700</td><td>56px *</td><td><code>nys-grid-gap-700</code></td><td><code>nys-grid-gap-x-700</code></td><td><code>nys-grid-gap-y-700</code></td></tr>
    <tr><td>800</td><td>64px *</td><td><code>nys-grid-gap-800</code></td><td><code>nys-grid-gap-x-800</code></td><td><code>nys-grid-gap-y-800</code></td></tr>
    <tr><td>1200</td><td>96px *</td><td><code>nys-grid-gap-1200</code></td><td><code>nys-grid-gap-x-1200</code></td><td><code>nys-grid-gap-y-1200</code></td></tr>
  </tbody>
</table>

<p>* Sizes 400 and above are capped on small screens to prevent overflow: horizontal gaps cap at <code>1rem</code> and vertical gaps cap at <code>2rem</code> below the <code>desktop</code> breakpoint (64em), then expand to their full values at <code>desktop</code> and wider.</p>

<p>Responsive breakpoint prefixes also work with all three axis variants (e.g. <code>nys-tablet:nys-grid-gap-x-200</code>).</p>

<div className="docs-grid-example">

<div className="nys-grid-row nys-grid-gap-400">
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
  <div className="nys-grid-col-4">
    <div>.nys-grid-col-4</div>
  </div>
</div>

</div>

```html
<!-- Both horizontal and vertical gap at size 400 -->
<div class="nys-grid-row nys-grid-gap-400">
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
  <div class="nys-grid-col-4"><div>.nys-grid-col-4</div></div>
</div>

<!-- Horizontal gap only -->
<div class="nys-grid-row nys-grid-gap-x-400">
  ...
</div>

<!-- Vertical gap only -->
<div class="nys-grid-row nys-grid-gap-y-400">
  ...
</div>
```
