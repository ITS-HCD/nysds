...
<nys-alert
  type="warning"
  heading="Modal display limited in Storybook & in Safari">
➤ YOU MUST <strong>CLOSE OUT OF THE MODAL</strong> IN ORDER TO RE-ACCESS THE SCROLLBAR

➤ <strong>DO NOT USE SAFARI</strong> to test this with storybook on Desktop!!
</nys-alert>
...
<div className="nys-grid-row nys-grid-gap-1">
  ...
  <p className="pill pill--uikit">
    <a href="https://www.figma.com/design/TODO/add/url" target="_blank">
      View in Figma
    </a>
  </p>
</div>
...
<code>transform</code> style, which prevents it from escaping the container.
...
<table>
  ...
  <tbody>
    <tr>
      <td>`actions`</td>
      <td>
        Place action buttons such as `nys-button` for confirm, cancel, or other
        choices
      </td>
      <td>none</td>
    </tr>
