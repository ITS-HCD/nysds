...
<div style={{ display: "flex", gap: "10px", verticalAlign: "middle", alignItems: "center" }}>
	{" "}
	<nys-icon size="xl" name="info"></nys-icon>The button component takes the property `onButtonClick`, which is a function that is called when the button is clicked.{" "}
</div>
```html
<nys-button onButtonClick={() => alert("Button clicked")} label="Click Me"></nys-button>
```
...
<div className="nys-grid-row nys-grid-gap-lg">
	<div className="nys-mobile-lg:nys-grid-col">
		<div className="usage-card usage-card--do">
			### <nys-icon name="check_circle" color="--var(nys-color-success)" /> Do - When users need to submit a form. - When users need to navigate to a different page. - When users need to trigger an action.
		</div>
	</div>
	<div className="nys-mobile-lg:nys-grid-col">
		<div className="usage-card usage-card--dont">
			### <nys-icon name="cancel" color="--var(nys-color-error)" /> Don't - Use inverted buttons on a light background. - Use buttons with icons only.
		</div>
	</div>
</div>
