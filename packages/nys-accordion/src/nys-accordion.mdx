...
### Expanded Item
The `expanded` boolean property is available on `nys-accordionitem`. When set, the content of the `nys-accordionitem` will be visible by default.
<Canvas of={NysAccordionStories.ExpandedItem} />

### Single Select
The `singleSelect` boolean property is available on `nys-accordion`. When set, only one `nys-accordionitem` in the wrapper can be open at a time.
<Canvas of={NysAccordionStories.SingleSelect} />
