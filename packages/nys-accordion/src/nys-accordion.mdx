...
The `nys-accordionitem` toggles open or closed via the `expanded` prop, which can be set manually. Wrapping it in a `nys-accordion` is optional since it is standalone.

**Note:** If you place multiple `nys-accordionitem` components side by side, wrap them in a nys-accordion.
...
### Bordered
The `bordered` prop is available on both `nys-accordion` and `nys-accordionitem`.
When applied to the `nys-accordion`, all `nys-accordionitem` within the wrapper will be bordered.
