...
export class NysAvatar extends LitElement {
	...
	set size(value: string) {
		this._size = NysAvatar.VALID_SIZES.includes(value as (typeof NysAvatar.VALID_SIZES)[number]) ? (value as (typeof NysAvatar.VALID_SIZES)[number]) : "lg";
		this.requestUpdate("size");
	}
	// Setter for the `shape` property.
	set shape(value: string) {
		this._shape = NysAvatar.VALID_SHAPES.includes(value as (typeof NysAvatar.VALID_SHAPES)[number]) ? (value as (typeof NysAvatar.VALID_SHAPES)[number]) : "circle";
		...
		return html`
			<label class="nys-avatar" id=${this.id}>
				<div class="nys-avatar__content">
					<div part="base" class="nys-component__component" style="color: ${this.color?.length > 0 ? this.color : "red"};">
						${this.image?.length > 0
							? html`<img part="image" class="nys-avatar__image" src=${this.image} alt=${this.label} loading=${this.lazy ? "lazy" : "eager"} />`
							: this.initials?.length > 0
								? html`<span part="initials" class="nys-avatar__initials">${this.initials}</span>`
								: this.icon?.length > 0
									? html`<div part="icon">
											<nys-icon label="nys-avatar__icon" name=${this.icon} size="md"></nys-icon>
										</div>`
									: html`<div part="icon">
											<slot name="icon">
												<nys-icon part="icon" label="nys-avatar__icon" name="account_circle" size="md"></nys-icon>
											</slot>
										</div>`}
					</div>
				</div>
