...
Excelsior components are built using Lit.dev and utilize the `shadowDOM.` While this approach is effective for encapsulating a component's internal structure, it prevents native `<form>` elements from recognizing form controls within a shadow root. \
To address this, Excelsior leverages the **`formdata` event**. 
...
In the example below, we demonstrate how to process the `FormData` object by listening to a native form submission event. The form includes both standard HTML form elements and custom `nys-components`.
```js
...
<Canvas of={NysFormStories.RequiredFields} />
