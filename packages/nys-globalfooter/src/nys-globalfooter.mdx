...
## With Column Links
Start by adding an unordered list (`<ul>`) inside the `nys-globalfooter` slot. \
For footers that require multiple sections or categories, group related links into columns. \
Each column should use a parent list item (`<li>`) with a heading (`<span>`) followed by a nested list (`<ul>`) of links.
