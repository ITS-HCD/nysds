...
export class NysTextinput extends LitElement {
	...
	private static readonly VALID_TYPES = ["email", "number", "password", "search", "tel", "text", "url"] as const;
	...
	set type(value: string) {
		this._type = NysTextinput.VALID_TYPES.includes(value as (typeof NysTextinput.VALID_TYPES)[number]) ? (value as (typeof NysTextinput.VALID_TYPES)[number]) : "text";
	}
			...
			<div class="nys-textinput">
				...
				html` <div class="nys-textinput__text">
					<div class="nys-textinput__label_labelwrapper">
						<label for=${this.id} class="nys-textinput__label">${this.label}</label>
						<label for=${this.id} class="nys-textinput__description">${this.description}</label>
					</div>
					${this.required && (this.label || this.description) ? html`<label class="nys-textinput__required">*</label>` : ""}
				</div>`}
				<div class="nys-textinput__requiredwrapper">
					<input class="nys-textinput__input ${this.size}" type=${this.type} name=${this.name} id=${this.id} ?disabled=${this.disabled} ?required=${this.required} ?readonly=${this.readonly} autocomplete=${this.autocomplete} aria-disabled="${this.disabled}" aria-label="${this.label} ${this.description}" .value=${this.value} placeholder=${this.placeholder} maxlength=${ifDefined(this.maxlength)} pattern=${ifDefined(this.pattern)} step=${ifDefined(this.step)} min=${ifDefined(this.min)} max=${ifDefined(this.max)} form=${ifDefined(this.form)} @input=${this._handleInput} @focus="${this._handleFocus}" @blur="${this._handleBlur}" @invalid=${this.handleInvalid} />
					${this.required && !this.label && !this.description ? html`<label class="nys-textinput__required">*</label>` : ""}
				</div>
			</div>
		`;
