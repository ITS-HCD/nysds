...
<Meta
  of={NysGlobalHeaderStories}
  title="Components/GlobalHeader/Documentation"
/>
...
<div className="nys-grid-row nys-grid-gap-1">
  ...
  <p className="pill pill--uikit">
    <a
      target="_blank"
      href="https://www.figma.com/design/U2QpuSUXRTxbgG64Fzi9bu?node-id=3981-11924"
    >
      View in Figma
    </a>
  </p>
...
>
  {" "}
  <nys-icon name="info" size="xl" style={{ margin: "20px 0" }}></nys-icon>{" "}
  Display only the agency's official name for public-facing sites that are not
...
>
  {" "}
  <nys-icon name="info" size="xl" style={{ margin: "20px 0" }}></nys-icon> When
  the application serves multiple agencies, use the application name instead of
  a specific agency name.
</div>
...
<div style={{ display: "flex", gap: "10px", verticalAlign: "middle" }}>
  {" "}
  <nys-icon name="info" size="xl" style={{ margin: "20px 0" }}></nys-icon>
  Applications using the Global Header typically do not include links in the
  application or agency name to avoid distractions and keep users on task.
  However, public-facing sites may include links, often within a collapsed menu
...
<div className="nys-grid-row nys-grid-gap-lg">
  <div className="nys-mobile-lg:nys-grid-col">
    <div className="usage-card usage-card--do">
      ...
      Use `agencyName` as stand alone if public-facing site and not in the
      context of a specific application. - Use `appName` if an application is
      for a single agency
    </div>
  </div>
  <div className="nys-mobile-lg:nys-grid-col">
    <div className="usage-card usage-card--dont">
      ...
      `appName` if an application is not for a single agency - Add unnecessary
