...
### Other
The `other` option lets users enter a custom value when none of the listed choices apply. When selected, a text input appears and the `nys-checkbox`’s `value` is set to the entered text.
...
<Canvas of={NysCheckboxStories.Other} />
### Inverted
...
<Canvas of={NysCheckboxStories.Inverted} />
