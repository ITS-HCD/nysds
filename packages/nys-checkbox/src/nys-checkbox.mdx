...
Note: The `tile` prop can be applied to the `nys-checkboxgroup` or the `nys-checkbox` component. If applied to the `nys-checkboxgroup`, all checkboxes will be displayed as tiles. If applied to the `nys-checkbox`, only that checkbox will be displayed as a tile.
Note: Do not use the `tile` prop on a checkbox if it is inside a `nys-checkboxgroup`. All checkboxes in a group should be the same size and style.
