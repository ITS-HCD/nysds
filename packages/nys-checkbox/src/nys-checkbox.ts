 ...
 * For grouping to work correctly, all `nys-checkbox` elements within the same **`nys-checkboxgroup`** must have the same name attribute.
 * This allows the form to recognize them as a related set of options.
 *
 * ## Variants and Options
 *
 * - **Size:** Set the `size` prop to `sm` (24px) or `md` (32px, default). Can be applied to the group for all checkboxes to be the same size.
 * - **Tile:** The `tile` prop changes the styling of the checkbox to a tile with a larger clickable area.
 *   Can be applied to the group or individual checkboxes, but do not use on checkboxes inside a group—all checkboxes in a group should be the same style.
 * - **Required:** Mark as required with the `required` prop, which validates that the checkbox is checked.
 * - **Optional:** Add the `optional` prop to add an optional flag to the input.
 * - **Error Message:** The error message appears only when the `showError` attribute is set to `true`. Setting `errorMessage` will not display the error message by default.
 * - **Disabled:** Prevent interaction with the `disabled` prop.
 * - **Slots:** When the description requires more complexity than a simple `string` value, add HTML content directly within the component as slot content
 *   rather than setting a `description` prop. This allows you to include HTML in the description, such as `<a>` or `<strong>`.
 ...
 * - **`nys-other-input`** – Fired when "other" text input value changes (for "other" checkboxes)
 *
