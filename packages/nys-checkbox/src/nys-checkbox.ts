	...
	render() {
		...
		return html`
			<label class="nys-checkbox">
				<input id="${this.id}" class="nys-checkbox__input" type="checkbox" name="${ifDefined(this.name ? this.name : undefined)}" .checked=${this.checked} ?disabled=${this.disabled} .value=${this.value} ?required="${this.required}" aria-checked="${this.checked}" aria-disabled="${this.disabled}" aria-required="${this.required}" @change="${this._handleChange}" @focus="${this._handleFocus}" @blur="${this._handleBlur}" @keydown="${this._handleKeydown}" />
				<div class="nys-checkbox__text">
					<div class="nys-checkbox__label">${this.label}</div>
					${this.description ? html`<div class="nys-checkbox__description">${this.description}</div>` : ""}
				</div>
			</label>
		`;
	}
