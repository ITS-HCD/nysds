...
<p class="pill uikit"><a href="https://www.figma.com/design/0ogYpymUPQQfhELthntNbt/%F0%9F%92%A0-NYS-%2F-Excelsior-Components-(Alpha)?node-id=1308-271&p=f&t=QF8BQ6f9JDt9M72a-0">Get the UI kit</a></p>

***

<Canvas of={NysAlertStories.Default} />
...
### Slim Variant
The slim variant `isSlim` is a compact version of the alert, ideal for short descriptions. It removes the header and centers the icon for a cleaner layout.
...
### When to use this component
  ...
  - Ideal for messages such as success notifications, error alerts, warnings, or informational updates.
  - Use for dismissible or auto-hiding alerts when the message is not critical to user workflow.
...
<div class="usage-wrapper">
   ...
   - Use the appropriate `theme` (e.g., `info`, `success`, `warning`, or `error`) to match the intent of the alert.
...
<table>
  ...
  <tbody>
    <tr>
      <td>`nys-alert__container`</td>
      <td>Wrapper for the entire alert, including the icon, text, and close button.</td>
    </tr>
    <tr>
      <td>`nys-alert__icon`</td>
      <td>Container for the alert's icon, optionally slimmed down based on the `isSlim` property.</td>
    </tr>
    <tr>
      <td>`nys-alert__text`</td>
      <td>Container for the alert’s text, including an optional heading and a slot for custom text.</td>
    </tr>
    <tr>
      <td>`nys-alert__label`</td>
      <td>Heading element for the alert, shown only when `isSlim` is false.</td>
    </tr>
    <tr>
      <td>`close-container`</td>
      <td>Wrapper for the close button, which is displayed when the alert is dismissible.</td>
    </tr>
    <tr>
      <td>`close-button`</td>
      <td>Button that triggers the alert's dismissal when clicked.</td>
    </tr>
  </tbody>
</table>