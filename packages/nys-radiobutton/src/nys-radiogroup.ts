...
export class NysRadiogroup extends LitElement {
	...
	set size(value: string) {
		...
		this._size = NysRadiogroup.VALID_SIZES.includes(value as (typeof NysRadiogroup.VALID_SIZES)[number]) ? (value as (typeof NysRadiogroup.VALID_SIZES)[number]) : "md";
	}
		...
		return html` <div role="radiogroup" class="nys-radiogroup">
			...
			html` <div class="nys-radiobutton__text">
				<div class="nys-radiobutton__requiredwrapper">
					<label for=${this.id} class="nys-radiogroup__label">${this.label}</label>
					${this.required ? html`<label class="nys-radiobutton__required">*</label>` : ""}
				</div>
