...
<p className="pill uikit"><a href="https://www.figma.com/design/0ogYpymUPQQfhELthntNbt/%F0%9F%92%A0-NYS-%2F-Excelsior-Components-(Alpha)?node-id=1436-996&p=f&m=dev">Get the UI kit</a></p>
...
- `md` (Medium): 200px, the default size for balanced designs.
- `lg` (Large): 384px, suitable for displaying longer content.
- `full` (Full Width): Expands to fill the available space, using responsive fill-available properties.
...
<div className="usage-wrapper">
    ...
    - Following the guidance of <a className="usage-link" target="_blank" href="https://designsystem.digital.gov/components/select/#when-to-consider-something-else">USWDS</a>, we recommend against using `<nys-select multiple/>` as an input for users to select multiple items from a dropdown. Instead use a checkbox group.
    </div>
