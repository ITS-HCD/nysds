...
<div className="nys-grid-row nys-grid-gap-1">
  ...
  <p className="pill pill--uikit">
    <a
      target="_blank"
      href="https://www.figma.com/design/U2QpuSUXRTxbgG64Fzi9bu?node-id=3981-9877"
    >
      View in Figma
    </a>
  </p>
...
<p>The `<nys-option>` component will be deprecated with the 2.0 release of the New York State Design System.</p>
<p>Use the native `<option>` tag for the options in the `<nys-select>` component.</p>
</nys-alert>
### Default Value
You can set a default value by adding the `selected` attribute to the desired `<nys-option>`.
<Canvas of={NysSelectStories.DefaultValue} />
### Option Group
You can separate the options by using the `<optgroup>` tag.
<Canvas of={NysSelectStories.OptionGroup} />
### Disabled
...
>
  {" "}
  <nys-icon name="info" size="xl" style={{ margin: "20px 0" }}></nys-icon> You
  must use `slot="description"` on your embedded HTML for it to work
  properly.{" "}
</div>
...
<Canvas of={NysSelectStories.Inverted} />
...
<div className="nys-grid-row nys-grid-gap-lg">
  <div className="nys-mobile-lg:nys-grid-col">
    <div className="usage-card usage-card--do">
      ...
      - Use the native `option` to define the options in the dropdown
