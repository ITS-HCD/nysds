...
<div class="nys-grid-row nys-grid-gap-1">
  ...
  <p className="pill pill--uikit">
    <a
      href="https://www.figma.com/design/U2QpuSUXRTxbgG64Fzi9bu?node-id=4897-5811"
      target="_blank"
    >
      View in Figma
    </a>
...
- This prop is automatically applied to all steps before the `current` step and is used to style the steps peoperly.
...
<div className="nys-grid-row nys-grid-gap-lg">
  ...
  <div className="nys-mobile-lg:nys-grid-col">
    <div className="usage-card usage-card--dont">
      ### <nys-icon name="error" color="var(--nys-color-danger)" /> Don't - Do
      not use the stepper if there are only 1 or 2 sections to the form. - Do
      not use the stepper for forms that are nonlinear and can be completed in
      any order.
    </div>
  </div>
</div>
