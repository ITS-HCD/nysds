...
import nyGovIdLogo from "./nygovid.logo";
...
export class NysNygovid extends LitElement {
  ...
  get signin() {
    return this.getAttribute("signin") || this._signin;
  }
  set signin(val) {
    this.setAttribute("signin", val);
  }

  get find() {
    return this.getAttribute("find") || this._find;
  }
  set find(val) {
  ...
  get create() {
    return this.getAttribute("create") || this._create;
  }
  set create(val) {
  ...
  render() {
    return html`
      ...
      <div class="login-card">
        <div class="logo-container">${unsafeSVG(nyGovIdLogo)}</div>
