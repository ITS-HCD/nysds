...
- Your agency's designated NYSDS contact`;
  ...
  // get_page_structure_guide - Standard page layout with NYSDS components
  server.tool(
    "get_page_structure_guide",
    "Get NYSDS page structure guide with required header/footer components and landmarks",
    {},
    async () => {
      const guide = `# NYSDS Page Structure Guide
...
- ❌ Placing \`nys-skipnav\` after other focusable elements
- ❌ Using \`href="#app"\` or wrapper div as skipnav target (use \`#main-content\`)
- ❌ Omitting the \`<main>\` landmark element
- ❌ Placing \`nys-backtotop\` inside the \`<footer>\` element
- ❌ Missing \`<footer>\` wrapper around footer components`;
