...
### Date Range
Use `minDate` and `maxDate` to set a range for date.
<Canvas of={NysDatepickerStories.CustomStartDate} />
...
- value (date): The currently value of the datepicker input. Return only when valid date is produced.
...
datepicker.addEventListener('nys-input', (event) => {
  ...
  console.log(`Date picker input (${id}) changed:`, value);
});
```
