#!/usr/bin/env node
/**
 * Generates Angular directive files for the "thin" UI wrappers from
 * `custom-elements.json`. Form (CVA) directives and special-case UI directives
 * remain hand-written — this script touches only the tags listed in
 * `GENERATED_TAGS` below.
 *
 * Output: `packages/angular/src/lib/ui/<tag>.directive.ts`
 *
 * Each generated file:
 *  - declares `selector: '<tag>'`, `standalone: true`
 ...
 * hand-written. To promote a directive to generator-managed, move its tag
...
function resolveType(typeText) {
  ...
  // Strip leading/trailing whitespace; CEM text is already a TS-style expression.
  return typeText.trim();
}

/** Filter to public @property fields the directive should expose. */
function isExposedField(member) {
  ...
  // Only fields backed by an attribute or explicitly declared as properties.
  // The CEM analyzer marks Lit @property fields with `attribute`; @state ones
  // typically lack it. Be permissive here: anything not private and not static.
  return true;
}
...
function main() {
  ...
  for (const tag of GENERATED_TAGS) {
    ...
    const source = buildDirectiveSource(tag, decl);
    const outPath = resolve(OUT_DIR, `${tag}.directive.ts`);
    writeFileSync(outPath, source);
    generated.push({ tag, outPath });
  }

  console.log(`Generated ${generated.length} directive files:`);
